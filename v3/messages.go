...
func (m *Messages[M]) LockForAdd() int64 {
	m.Lock()
	return m.offset + int64(len(m.messages))
}
...
func (m *Messages[M]) GetDrop() (drop, dropOffset int64) {
	m.RLock()
	defer m.RUnlock()
	if len(m.messages) == 0 {
		return
...
func (m *Messages[M]) Drop(drop int64) {
	m.Lock()
